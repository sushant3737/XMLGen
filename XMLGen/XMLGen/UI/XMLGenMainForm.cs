...
namespace XMLGen.UI
{
    ...
    public partial class XMLGen : Form
    {
        ...
        {
           KeyValue kval = (KeyValue) dgview_FieldEdit.CurrentRow.DataBoundItem;
            try
            {
                switch (kval.ValueType)
                {
                    case "byte":
                        byte.Parse(kval.Value.ToString());
                        break;
                    case "sbyte":
                        sbyte.Parse(kval.Value.ToString());
                        break;
                    case "short":
                        short.Parse(kval.Value.ToString());
                        break;
                    case "ushort":
                        ushort.Parse(kval.Value.ToString());
                        break;
                    case "int":
                        int.Parse(kval.Value.ToString());
                        break;
                    case "uint":
                        uint.Parse(kval.Value.ToString());
                        break;
                    case "long":
                        long.Parse(kval.Value.ToString());
                        break;
                    case "ulong":
                        ulong.Parse(kval.Value.ToString());
                        break;
                    case "bool":
                        bool.Parse(kval.Value.ToString());
                        break;
                        //case "string":
                        //    string typ8;
                        //    string.TryParse(kval.Value.ToString(), out typ8);
                        //    break;

                }
                toolStripStatusLabel_AppStatus.Text = "";
            }
            catch (Exception)
            {
                kval.Value = "";
                toolStripStatusLabel_AppStatus.Text = "Enter valid value";
            }
        ...
        private void ToolStripMenuItem_Exportbinary_Click(object sender, EventArgs e)
        {
            try
            {
                if (Serialization.Serialization.Serialize<Dictionary<int, Dictionary<int, List<KeyValue>>>>(MessageInfo, @"\Binarydata\Messages.dat"))
                {
                    List<KeyValue> mcopy = MessageProperty.MsgAttriblist.Select(o => new KeyValue
                    {
                        Key = o.Key,
                ...
                {
                    toolStripStatusLabel_AppStatus.Text = "DAT File export unsuccessfull";
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
        ...
        private void xMLToolStripMenuItem_export_Click(object sender, EventArgs e)
        {
            try
            {
                if (XmlGenr.XMLSerialize<Dictionary<int, Dictionary<int, List<KeyValue>>>>(MessageInfo, @"\XMLData\Messages.xml"))
                {
                    toolStripStatusLabel_AppStatus.Text = "XML File exported successfully";
                }
                else
                {
                    toolStripStatusLabel_AppStatus.Text = "XML File export unsuccessfull";
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
                    ...
                    case 2:
                        {
                            int ParentNodeindex = Messages_TreeView.SelectedNode.Parent.Index;
                            int SelectedfldNodeindex = Messages_TreeView.SelectedNode.Index;
                            if (MessageInfo[ParentNodeindex].ContainsKey(SelectedfldNodeindex))
                            {
                                var tempdict = MessageInfo[ParentNodeindex].Where(x => x.Key > SelectedfldNodeindex + 1).ToDictionary(p => p.Key - 1);
                                foreach (var ele in tempdict)
                                {
                                    MessageInfo[ParentNodeindex].Remove(ele.Key);
                                    MessageInfo[ParentNodeindex].Add(ele.Key, ele.Value.Value);
                                }
                                MessageInfo[ParentNodeindex].Remove(MessageInfo[ParentNodeindex].Count - 1);
                            }
                            UpdateTreeView();
                        }
                        break;
